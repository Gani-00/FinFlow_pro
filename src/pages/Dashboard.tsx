...
import { Plus, PieChart, Target, FileText, Settings, ChevronRight, TrendingUp, TrendingDown, Sparkles, Bell } from "lucide-react";
...
import { setDate, isBefore, startOfToday, addMonths, startOfMonth } from "date-fns";
  ...
  useEffect(() => {
    ...
    const unsubscribeTx = onSnapshot(txQuery, (snapshot) => {
      ...
      txs.forEach((tx: any) => {
        ...
        if (tx.type === 'income') totalInc += amount;
        else totalExp += amount;

        if (txDate >= startOfThisMonth) {
          if (tx.type === 'income') monthlyInc += amount;
          else monthlyExp += amount;
        }
      });

      const spentPercentage = monthlyInc > 0 ? (monthlyExp / monthlyInc) * 100 : 0;
    ...
    const unsubscribeBills = onSnapshot(billsQuery, (snapshot) => {
      const bList = snapshot.docs.map(doc => {
        ...
        let nextDate = setDate(new Date(), day);
        if (isBefore(nextDate, startOfToday())) {
          nextDate = addMonths(nextDate, 1);
        }
        return { id: doc.id, ...data, nextDate };
      });
      // Sort by next date
      const sorted = bList.sort((a, b) => a.nextDate.getTime() - b.nextDate.getTime());
      setBills(sorted);
    });
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="flex justify-between items-start">
        <div>
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none mb-1 text-left">HI, {auth.currentUser?.displayName?.toUpperCase() || "YOU"}</p>
      ...
      <div className="relative overflow-hidden group">
        ...
        <div className="relative p-8 text-white space-y-6">
          <div className="space-y-1">
             <p className="text-[10px] font-bold uppercase tracking-widest opacity-80">Current Balance</p>
             <h2 className="text-5xl font-bold tracking-tighter">{formatCurrency(stats.totalBalance, currency)}</h2>
          </div>
          ...
          <div className="flex gap-10">
            <div>
              ...
              <p className="text-xl font-bold">{formatCurrency(stats.income, currency)}</p>
            </div>
            <div>
              <p className="flex items-center gap-1.5 text-[10px] uppercase font-bold tracking-wider opacity-70 mb-1">
                <TrendingUp size={10} /> Spent
              </p>
              <p className="text-xl font-bold">{formatCurrency(stats.expenses, currency)}</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          { label: 'Planning', icon: Target, path: '/planning', color: 'bg-rose-50 text-rose-500' },
          { label: 'Coach', icon: Sparkles, path: '/ai-advisor', color: 'bg-cyan-50 text-cyan-500' },
      ...
      {budgets.length > 0 && (
        <section className="space-y-4">
          <div className="flex justify-between items-center px-1">
            <h3 className="text-lg font-bold text-gray-900 leading-none">Your Budgets</h3>
            <button onClick={() => navigate('/planning')} className="text-xs font-bold text-[#7c3aed] uppercase tracking-widest">View All</button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {budgets.map((budget) => {
              const startOfThisMonth = startOfMonth(new Date());
              const spent = transactions
                .filter(t => t.type === 'expense' && t.category === budget.category && new Date(t.date) >= startOfThisMonth)
                .reduce((acc, t) => acc + Number(t.amount), 0);
              const progress = Math.min((spent / budget.limit) * 100, 100);
              return (
                <div key={budget.id} className="bg-white border border-gray-100 rounded-[32px] p-6 shadow-sm">
                  <div className="flex justify-between items-center mb-3">
                    <span className="font-bold text-sm text-gray-900">{budget.category}</span>
                  ...
                  <div className="h-1.5 w-full bg-gray-50 rounded-full overflow-hidden">
                    <motion.div 
                      ...
                      className={`h-full ${progress > 90 ? 'bg-rose-500' : 'bg-emerald-500'} rounded-full`}
                    />
                  </div>
                </div>
              );
        ...
        <section className="space-y-4">
          ...
          >
             ...
             <div className="flex-1">
                <h4 className="font-bold text-gray-900">{formatCurrency(totalMonthlyUpcoming, currency)} due this month</h4>
          ...
          {bills.slice(0, 2).map((bill, i) => (
            ...
            >
              ...
              <div className="flex-1">
                  ...
                  <p className="text-lg font-bold tracking-tight">{formatCurrency(bill.amount, currency)}</p>
